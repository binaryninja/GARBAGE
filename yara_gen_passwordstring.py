...
string_rules = []
yara_template = """yara_template.yara
 rule base64_admin
 {
    meta:
...
for word in wordlist:
    ...
    sha512.update(word)    
    #print md5.hexdigest()
    string_rules.append( "$"+word+"_MD5 = \"" + md5.hexdigest() + "\"\n")
    string_rules.append( "$"+word+"_SHA1 = \"" + sha1.hexdigest() + "\"\n")
    string_rules.append( "$"+word+"_SHA256 = \"" + sha256.hexdigest() + "\"\n")
    string_rules.append( "$"+word+"_SHA512 = \"" + sha512.hexdigest() + "\"\n")
    string_rules.append( "$"+word+"_BASE64 = \"" + base64.b64encode(word) + "\"\n")
print yara_template % " ".join(string_rules)
